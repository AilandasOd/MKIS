...
const MapWithDrawing: React.FC = () => {
  ...
  const [polygonData, setPolygonData] = useState<{ name: string; coordinates: google.maps.LatLngLiteral[] } | null>(null);
  ...
  const loadExistingPolygons = (mapInstance: google.maps.Map) => {
    ...
    // Load hunting area (red polyline)
  ...
  const handleSavePolygon = async (name: string, coordinates: google.maps.LatLngLiteral[]) => {
    ...
    try {
      ...
      setPolygonData({ name, coordinates });
